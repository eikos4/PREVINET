...
import { getIRLs } from "./irl.service";
import type { IRL } from "./irl.service";
import { downloadBlobAsFile, getSignedIrlPdfByKey } from "./irlPdf.service";
import { generateMasterIrlPdf, buildMasterIrlPdfFileName } from "./irlMasterPdf.service";


export default function IRLList() {
  const [irls, setIrls] = useState<IRL[]>([]);
  const [downloadError, setDownloadError] = useState<string>("");
  const [downloadingKey, setDownloadingKey] = useState<string | null>(null);
  const [expandedIrlIds, setExpandedIrlIds] = useState<Set<string>>(() => new Set());

  useEffect(() => {
    getIRLs().then((list) => setIrls(list));
  }, []);
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-8 py-6">
            <h3 className="text-2xl font-bold text-white flex items-center gap-3 m-0">
              ...
              <span>IRL publicados</span>
            </h3>
          </div>

          <div className="p-12 text-center">
            <p className="text-gray-400 text-base m-0">
              No hay IRL publicados aún.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            {sorted.map((i) => {
              const total = i.asignados.length;
              const firmados = i.asignados.filter((a) => !!a.firmadoEn).length;
              const pendientes = total - firmados;
              const porcentaje = total > 0 ? Math.round((firmados / total) * 100) : 0;

              const signedAssignments = i.asignados.filter((a) => !!a.firmadoEn);
              const isExpanded = expandedIrlIds.has(i.id);
                    ...
                    <div className="flex items-start gap-4 justify-between">
                      <div className="flex-1">
                        ...
                        {i.attachment?.fileName && (
                          <p className="text-sm text-gray-600 mb-3 m-0">
                            Documento: <span className="font-medium">{i.attachment.fileName}</span>
                          </p>
                        )}
                      ...
                      <div className="mb-4 flex gap-2">
                        <button
                          ...
                          onClick={async (e) => {
                            e.stopPropagation();
                            try {
                              const pdf = await generateMasterIrlPdf(i);
                              const fileName = buildMasterIrlPdfFileName(i);
                        ...
                        <>
                          ...
                          <div className="space-y-3">
                            {signedAssignments.map((a) => {
                              ...
                              const key = `${i.id}_${a.workerId}_${a.token}`;
